...
velini = 100.0 #velocidade inicial
dt = 0.0002
msh = meshio.read("separada.msh")
...
trs = msh.cells_dict["triangle"]
cc_data = msh.cell_data_dict["gmsh:physical"]["line"]

trs_data = msh.cell_data_dict["gmsh:physical"]["triangle"]
...
for i in range(0, len(cc)):
    if cc_data[i] == 1 or cc_data[i] == 2 or cc_data[i] == 3 or cc_data[i] == 4 or cc_data[i] == 9:
        cc_esp.append([i])

cc_esp = np.array(cc_esp)
inner = [x for x in range(0,npoints) if x not in cc_esp]
...
for i in range(0, len(cc)):
    if cc_data[i] == 1 or cc_data[i]==3:
        vx[cc[i][0]] = velini
        vx[cc[i][1]] = velini
        vy[cc[i][0]] = 0
        vy[cc[i][1]] = 0
    if cc_data[i] == 2 or cc_data[i] == 4 or cc_data[i] == 9:
        vx[cc[i][0]] = 0
        vx[cc[i][1]] = 0
        vy[cc[i][0]] = 0
        vy[cc[i][1]] = 0
...
# # zerando os valores de omegacc no interior da malha
    ...
    for i in range(0, len(cc)):
        if cc_data[i] == 1 or cc_data[i] == 2 or cc_data[i] == 3 or cc_data[i] == 4 or cc_data[i] == 9:
            csr_zero_rows(A,cc[i][0])#zerando linha
            csr_zero_rows(A,cc[i][1])#zerando linha

            csr_zero_rows(A_2,cc[i][0])#zerando linha
            csr_zero_rows(A_2,cc[i][1])#zerando linha
...
# resolve eq. transporte solve
    ...
    for i in range(0, len(cc)):
        if cc_data[i] == 1 or cc_data[i] == 2 or cc_data[i] == 3 or cc_data[i] == 4:
            psicc[cc[i][0]] = Y[cc[i][0]]*velini
            psicc[cc[i][1]] = Y[cc[i][0]]*velini
        if cc_data[i] == 9:
            psicc[cc[i][0]] = 0
            psicc[cc[i][1]] = 0
        else:
...
# encontrar velocidades M vx = Gy \psi, M vy = -Gx \psi
 ...
 #impondo cc para vx e vy
    ...
    somay = 0
    for i in cc_F:
        det = X[i[0]]*( Y[i[1]] - Y[i[2]]) \
              + X[i[1]]*( Y[i[2]] - Y[i[0]]) \
              + X[i[2]]*( Y[i[0]] - Y[i[1]])
        area = abs(det)/2
        xc1 = ( X[i[0]] + X[i[1]] + X[i[2]] )/3
        yc1 = ( Y[i[0]] + Y[i[1]] + Y[i[2]] )/3
        omegamed = ( omega[i[0]] + omega[i[1]] + omega[i[2]] )/3
        somay += -xc1*area*omegamed
    if INTE%1==0:
